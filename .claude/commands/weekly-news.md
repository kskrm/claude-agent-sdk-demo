...
各件の図の種類と要素技術の数をログに出す。**そのログを必ず確認すること。**

- 「図: なし」が出た件は、図の仕様が壊れている。その 1 件だけ再度起動する。
- **5 件すべてが同じ図の型（例: 全部 `compare`）になっていたら**、内容的に `flow` や `layers` の
  ほうが正確なものを 1〜2 件選び、型を指定して書き直させる。図が全部同じだと読み手が飽きる。
...
- 深掘り 5 本の見出し
- 図の型の内訳（例: flow 2 / compare 2 / layers 1）
- 公開 URL
- Discord 通知の成否
