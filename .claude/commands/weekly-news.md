...
```json
{
  ...
  "keyPoints": [
    { "label": "見出し（10字以内）", "icon": "アイコン名", "text": "要点を 50 字以内で" }
  ]
}
```

`keyPoints` は **ちょうど 4 個**。中央の「今週の要点」を囲む 4 象限として表示されるので、
4 個でないと紙面が崩れる。`label` は 10 字以内、`text` は 50 字以内で切ること。

`icon` は `.claude/agents/topic-explainer.md` のアイコン名一覧から選ぶ。
一覧外の名前は `merge.mjs` が捨てて既定のアイコンに差し替える。
